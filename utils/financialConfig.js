...
export function parseDateEffet(raw) {
    if (!raw)
        return '';
...
export function configEffectDate(c) {
    const fromEffet = parseDateEffet(c.dateEffet);
    if (fromEffet)
        return fromEffet;
    if (!c.updatedAt)
        return '';
    const s = typeof c.updatedAt === 'string' ? c.updatedAt : new Date(c.updatedAt).toISOString();
    return s.slice(0, 10);
}
//# sourceMappingURL=financialConfig.js.map