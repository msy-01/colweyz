...
export const Drivers: React.FC<DriversProps> = ({ currentUser }) => {
  ...
  const handleSave = async () => {
    ...
    try {
        ...
        const allDrivers = drivers; // available in state
        
        // 1. Vérifier doublon par téléphone
        const phoneExists = allDrivers.some(d => d.phone === telephone && d.id !== editingDriver.id);
        if (phoneExists) {
        ...
        if (identifiant) {
          const usernameExists = allDrivers.some(d => d.username === identifiant && d.id !== editingDriver.id);
          if (usernameExists) {
        ...
        // Note: Firebase Auth disabled completely in migration to SQL.
        // Auth creation logic is removed. Driver auth is handled exclusively by SQL via the API.
  ...
  const calculateCurrentBalance = (driver: Driver) => {
      // Solde sur la période sélectionnée (aligné avec la vue « jour » de l’ancienne app)
      ...
      const myOrders = orders.filter((o) => {
        ...
        const raw = o.deliveredAt || o.date || '';
        if (!raw) return false;
        const orderDay = new Date(String(raw).split('T')[0]);
        if (Number.isNaN(orderDay.getTime())) return false;
        return isWithinInterval(orderDay, { start, end });
      });
      ...
      // Période : espèces collectées − commissions (sans initialBalance cumulé)
      return totalCashCollected - totalRemuneration;
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {drivers?.filter((d) => d.id !== DEPOT_ID).map(driver => {
          const isConfirming = deleteConfirmId === driver.id;
          const currentBalance = calculateCurrentBalance(driver);
          
          // > 0 : espèces collectées non reversées sur la période (dû à Colweyz)
          ...
          <div key={driver.id} className="bg-white p-6 rounded-xl shadow-sm border border-gray-100 relative group">
            ...
            <div className="mb-4 flex items-center gap-2">
                ...
                {!driver.uid && currentUser?.role === 'super_admin' && (
                    <button 
                        onClick={() => {
                            const uid = prompt("Entrez l'UID Système pour ce livreur (ou laissez vide pour annuler):");
                            if (uid) {
