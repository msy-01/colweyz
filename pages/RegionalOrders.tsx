...
/** Évite qu’un poll lent n’écrase une modification locale plus récente. */
function mergeRegionalOrdersByUpdatedAt(prev: Order[], incoming: Order[]): Order[] {
  const prevById = new Map(prev.map((o) => [o.id, o]));
  return incoming.map((serverOrder) => {
    const local = prevById.get(serverOrder.id);
    if (!local?.updatedAt || !serverOrder.updatedAt) return serverOrder;
    return new Date(local.updatedAt) > new Date(serverOrder.updatedAt) ? local : serverOrder;
  });
}
...
export const RegionalOrders: React.FC<RegionalOrdersProps> = ({ currentUser }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = DataService.subscribeToRegionalOrders((regional) => {
        setOrders((prev) => mergeRegionalOrdersByUpdatedAt(prev, regional));
    });
  ...
  const stockDepot = useMemo(() => {
    ...
    return depotEntries.map(e => {
      ...
      const siValue = e.SI ?? (e as any).si ?? 0;
      const currentSF = siValue + (e.entrees || 0) - (e.sorties || 0) + ajustementManuel;
