...
export const Deliveries: React.FC<DeliveriesProps> = ({ currentUser }) => {
  ...
  /** En mode API : montant calculé côté serveur (évite écarts mode de paiement / filtre date). */
  const [serverDebt, setServerDebt] = useState<number | null>(null);
  ...
  useEffect(() => {
    ...
    const q =
      selectedDriverId !== 'all'
        ? `?driverId=${encodeURIComponent(selectedDriverId)}`
        : '';
    api
      .get<{ amountDueColweyz: number }>(`/stats/colweyz-debt${q}`)
      .then((r) => setServerDebt(r.amountDueColweyz))
      .catch(() => setServerDebt(null));
  }, [selectedDriverId, orders.length]);
  ...
  const stats = useMemo(() => {
    ...
    // Solde global (toutes dates) — identique à Balances ; indépendant du filtre date ci-dessus
    const balance = (initial + totalRemun) - totalCash;
    const amountDueColweyz = balance < 0 ? Math.abs(balance) : 0;

    const due =
      serverDebt !== null ? serverDebt : amountDueColweyz;
    return { totalCA, totalCommissions, projectedRevenue, totalCourses, amountDueColweyz: due };
  }, [filteredOrders, orders, drivers, selectedDriverId, serverDebt]);
